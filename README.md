...
Il y a également un autre fichier, utilisé pour l'histogramme des précipitations. Il s'agit de "moyennes_prcp_1991-2020.csv". Comprenant 312 lignes, il est divisé en cinq colonnes :
...
Chaque canton se voit attribué de douze lignes, une pour chaque mois. Si le mois est la seule information pertinente de la colonne *date*, la conservation du format année-mois-jour nous a permis de ne pas avoir à perturber le code initial pour s'y référer. Les autres colonnes, à part *prcp*, servent pour compléter les informations sur la provenance de ces valeurs, comme dans le premier fichier csv. Il était important de garder un modèle similaire, afin de permettre au lecteur de facilement s'y retrouver.
...
Les 4 cercles de la spider-chart représente une échelle représentant, du plus petit au plus grand cercle, 25, 50, 75 et 100%. Si cette échelle s'applique pour les quatres mesures, leurs données très disparatre d'une colonne à une autre impliquèrent l'utilisation d'une échelle pour chaque mesure. Le 100% représente la valeur maximale d'une colonne. Si initialement, le principe était d'utiliser la valeure minimale comme point de départ dans la visualisation, il s'avère que cela créait des problèmes, puisque cela ne prenait pas en compte les cases sans données, cf.[image_problème]. Ainsi, au lieu d'établir la valeur minimale à 0, le but fut d'établir des moyennes avec des valeurs minimales et avec des valeurs maximales, mais en référant au fichier-même, et non manuellement.
...
[site_mouseover](https://medium.com/@kj_schmidt/show-data-on-mouse-over-with-d3-js-3bf598ff8fc2)# Données météorologiques 2025
